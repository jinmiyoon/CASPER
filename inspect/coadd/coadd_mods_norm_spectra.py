...
## This script does normalize mods spectra with GISIC first and cross-correlate mods1 and mods2 to align wavelength 
## for coadding the two spectra.
...
#star_name = "g77-61"
    ...
    spec1_utils = Spectrum1D(spectral_axis=wave*u.AA, flux=norm_flux1*u.photon,uncertainty=uncertainty, velocity_convention='optical', rest_value=4320. *u.AA)
    spec2_utils = Spectrum1D(spectral_axis=wave*u.AA, flux=norm_flux2*u.photon,uncertainty=uncertainty, velocity_convention='optical')
    ...
    plt.text(4300, 1.05, "RV shift "+str(round(lag[maxind].value, 3))+ " km/s" )
    plt.xlim(3900, 4400)
    plt.title(star_name+" cross-correlation")
    plt.legend()
    plt.savefig(star_name+'_cross-correlated-shift.pdf')
    plt.close()

    plt.plot(new_wave, coadd_flux/2.,  lw=0.5,label= "coadd")
    plt.plot(new_wave, norm_coadd_flux,  lw=0.5, label ='GISIC coadd')
    plt.axhline(1.0)
    plt.title(star_name+" coaddition of m1b and m2b")
    plt.xlim(3900, 4400)
    plt.legend(loc=4)
    plt.savefig(star_name+'_coadd-norm-spectrum.pdf')
    plt.close()

    ascii.write([new_wave, coadd_flux], star_name+"_coadd_spectrum.csv",
