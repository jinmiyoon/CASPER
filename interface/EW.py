...
def GBAND_QUAD(wave, flux, bounds = [4222, 4322]):
    ...
    ## is indeed lower than 1.0 level. However, I just leave the below scripts as it is for the future debugging.
    ...
    ## I need to use SNR at CH band for noise = 1./SNR but it appears very small (~0.02) so at the moment I ignore this.
    ...
    # I added EW_subtract in return so that I can use this subtraction for set_CH_procedure() J. Yoon, 07/17/2020
...
def set_CH_procedure(spectrum):
    ...
    # To do so I define flux_bounds_max in GBAND_QUAD and calculate this area and feed this number in this procedure.
    #
    #reduced_CH_EW = CH_EW - EW_subtract
    #if reduced_CH_EW > 45.:
    # I decided to keep Devin's procedure because the synthetic spectra at this band indeed lower than 1.0 level.
    ##########################################################################################################
    
    if CH_EW > 55.:  # BTW, I need to find a reason this value is reasonable.
